...
import { searchUnified } from "@/services/nlpSearch";
...
export default function SearchDetails({
  ...
  const personalizedChips = useMemo(() => {
    ...
    if (user && profile) {
      ...
      if (profile.preferredCity) {
        const cityName = profile.preferredCity
          .replace(/TP\.|Thành phố\s*/i, "")
          .trim();
      ...
      if ((profile as any).preferredDistricts?.length) {
        (profile as any).preferredDistricts
          .slice(0, 2)
      ...
      if (profile.preferredWards?.length) {
        const normalizeWardLabel = (w: string) =>
          w?.replace(/^(phường|phuong|xã|xa|thị trấn|thi tran)\s*/i, "").trim();
        profile.preferredWards.slice(0, 2).forEach((ward) => {
          const wardLabel = normalizeWardLabel(ward);
          if (wardLabel) chips.unshift(`Phường ${wardLabel}`);
        });
      }
  ...
  const performSearch = useCallback(
    async (queryValue: string, signal?: AbortSignal | null) => {
      try {
        ...
        const mapAmenity = (text: string) => {
          ...
          if (t.includes("điện nước") || t.includes("bao điện nước"))
            return "bao_dien_nuoc";
          if (t.includes("thang máy")) return "thang_may";
          if (
            t.includes("đỗ xe") ||
            t.includes("chỗ đỗ xe") ||
            t.includes("bãi xe")
          )
            return "bai_do_xe";
          return undefined;
        ...
        const mapCategory = (text: string) => {
          const t = text.toLowerCase();
          if (t.includes("phòng trọ")) return "phong-tro";
          if (t.includes("chung cư")) return "chung-cu";
          if (t.includes("nhà nguyên căn")) return "nha-nguyen-can";
          return undefined;
        };
        ...
        if (activeFilters.rentType && activeFilters.rentType !== "Tất cả") {
          ...
          params.set("postType", t.includes("ở ghép") ? "roommate" : "rent");
        }
        // roomType -> category
        ...
        if (activeFilters.furniture && activeFilters.furniture !== "Tất cả") {
          ...
          if (f.includes("có nội thất")) params.set("furniture", "full");
          else if (f.includes("không nội thất"))
            params.set("furniture", "none");
          else if (f.includes("bán nội thất")) params.set("furniture", "basic");
        }
        // demand -> gender (cho ở ghép)
        if (activeFilters.demand && activeFilters.demand !== "Tất cả") {
          const d = activeFilters.demand.toLowerCase();
          if (d === "nam") params.set("gender", "male");
          else if (d === "nữ" || d === "nu") params.set("gender", "female");
        ...
        const json = await searchUnified(params, {
          signal: signal || undefined,
        });
        const data = json?.data || {};
        const items = Array.isArray(data.items) ? data.items : [];
        const prefetch = Array.isArray(data.prefetch) ? data.prefetch : [];
        const prefetchItems = prefetch.flatMap((p: any) =>
          Array.isArray(p?.items) ? p.items : []
        );

        // Ghép prefetch để user không phải đợi khi lật trang (soft, dedupe)
        const mergedItems = (() => {
          const seen = new Set<string>();
          const out: any[] = [];
          const add = (arr: any[]) => {
            arr.forEach((it) => {
              const key = `${it?.type || it?.category || "x"}-${
                it?.id || it?.postId || it?.roomId || Math.random()
              }`;
        ...
        const total =
          typeof data.total === "number"
            ? data.total
            : mergedItems.length || items.length || 0;
        ...
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("app:search-result", {
              detail: {
                query: finalQuery,
                items: mergedItems,
                suggestions,
  ...
  const emitSearchEvent = (value: string) => {
    ...
    window.dispatchEvent(
      new CustomEvent("app:nlp-search", {
        detail: {
          q: trimmed,
  ...
  const handleSearch = () => {
    ...
    if (selected.length > 0) {
      setRecentSearches((prev) => {
        const newSearches = [
          ...selected,
          ...prev.filter((item) => !selected.includes(item)),
        ];
        const limited = newSearches.slice(0, 5);
        // Lưu vào localStorage
        if (typeof window !== "undefined") {
  ...
  useEffect(() => {
    ...
    // Chỉ chạy một lần khi mount để tránh loop setState
  ...
  // Khi chips thay đổi, cập nhật URL và trigger search (tránh gọi trong render)
  useEffect(() => {
    if (!mounted) return;
    pushQueryToUrl(q);
    emitSearchEvent(q);
  ...
  return (
    <section className="relative bg-gray-50 pt-12 pb-6" data-search-details>
      <div className="mx-auto max-w-7xl px-6 lg:px-8 w-full">{content}</div>
    </section>
  );
}
